...
#include "stm32f4xx.h"
#include "FreeRTOS.h"
//TODO: next two lines only needed to toggle LED, remove them
...
static inline int rb_putc(struct ringbuf *rb, const char data)
{
    if (rb->len >= rb->bufsize)
    	//rb->len = rb->bufsize; // instead of waiting, overwrite oldest data in circular fashion
        return 0; // ensures that data is removed before adding more
