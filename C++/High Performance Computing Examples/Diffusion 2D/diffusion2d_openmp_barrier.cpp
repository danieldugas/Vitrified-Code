...
public:
    ...
    : D_(D), L_(L), N_(N), Ntot(N_*N_), dt_(dt), t_(0)
    {
		...
		{        
			...
			initialize_density( id * N_ / nthreads , (id + 1) * N_ / nthreads );
    	}
    }
    ...
    void advance( const size_type start , const size_type end )
    {
        ...
        #pragma omp barrier
		#pragma omp single // single imposes synchronization at the end
		{
...
private:
    ...
    void initialize_density(const size_type start , const size_type end)
    {
...
int main(int argc, char* argv[])
{
    ...
    system.write_density("density_openmp_barrier_000.dat");
    ...
    #pragma omp parallel
    {
        ...
        while (time < tmax) {
            system.advance( id * N / nthreads , (id + 1) * N / nthreads );
    ...
    system.write_density("density_openmp_barrier.dat");
